...
export default function EditProfile() {
    ...
    async function updateProfile(d) {
        ...
        if((d.username == "" || d.username == userDetails.username) && d.password == "" && d.profilePicture.length == 0){
            return;
        }
        ...
        <form onSubmit={handleSubmit(updateProfile)} id='update-profile-form' className='flex flex-col mt-2 items-center gap-8 sm:w-[400px] w-[80%]'>
            ...
            <div className='relative w-full'>
                <div className="relative">
                    <input type="text" placeholder=" " defaultValue={userDetails?.username || ''} className='w-full bg-[#030E1E] px-3 pt-5 pb-2 rounded-md border-[1px] border-[#4d4d4d] text-[#A4A4A4] peer placeholder-transparent focus:outline-none' {...register("username")} />
                    <label  className='absolute text-base left-3 top-3 text-[#A4A4A4] transition-all duration-200 peer-focus:text-xs peer-focus:top-1.5 peer-[:not(:placeholder-shown)]:text-xs peer-[:not(:placeholder-shown)]:top-1.5'>Username</label>
                </div>
                {errors.username && <div className='text-red-600 mt-1'>{errors.username.message}</div>}
            </div>
            <div className='relative w-full'>
                <div className="relative">
                    <input type="password" placeholder=" " className='w-full bg-[#030E1E] px-3 pt-5 pb-2 rounded-md border-[1px] border-[#4d4d4d] text-[#A4A4A4] peer placeholder-transparent focus:outline-none'
                        {...register("password", {
                            validate: (value) => {
